<template>
 <div class="registration-form__wrapper">
  <div class="form__inner form-container ui segment piled">
   <b-form class="login-form ui form" action="https://vuejs.org/" method="post">
    <h1>{{ title }}</h1>
    <div class="field">
     <b-form-group
      id="input-group-1"
    ...
    <div class="field">
     ...
     />
     <small v-if="errors.login" class="error"
      >Заполните {{ passwordTitle }}</small
     >
    </div>
...
export default {
 ...
 methods: {
  ...
  registerForm() {
   ...
   let passwordUser = this.password;
   ...
   usersCollection
    ...
    .then((foundUsers) => {
     let user;
     let result = foundUsers.docs.some(function (doc) {
      user = doc.data();
