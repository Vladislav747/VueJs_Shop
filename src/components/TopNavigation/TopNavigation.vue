  ...
  <div class="max-width-block header-inner">
   ...
   <div class="icons-block">
    <div class="icon-wrapper cart-link">
     <router-link title="Корзина" id="cart" :to="{ name: 'cart' }">
      <div v-if="hasCartItems" class="cart-link__count">{{ cartCount }}</div>
      <font-awesome-icon icon="shopping-cart" size="2x" class="cart-icon" />
     </router-link>
    </div>

    <div class="icon-wrapper">
     <div class="profile-container" v-if="isLogined">
      <b-avatar
       variant="info"
       ...
       class="mr-3"
      />
      <b-button class="btn-logout" variant="primary" @click="logout()"
       >Выйти</b-button
      >
     </div>

     <div class="loginSection" v-else>
      <router-link title="Войти" id="login-form" :to="{ name: 'login-form' }">
...
export default {
 ...
 computed: {
  ...
  hasCartItems: function () {
   let localCartItems = localStorage.getItem("totalItems");
   //Проверяем данные либо из VUEX контейнера либо из localStorage
   let cartItemsCount = localCartItems ? localCartItems : this.cartItems;
   if (cartItemsCount > 0) {
  ...
  isLogined() {
   const loginedLocal = localStorage.getItem("isLogined");
   if (loginedLocal) {
